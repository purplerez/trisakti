            ...
            <div class="table-lap mt-4">
                <table class="custom-table">
                    ...
                    if (isset($_GET['tanggal'])) {
                        ...
                        $data = $transaksi->view_transaksi_by_date($tanggal);
                    } else {
                    ...
                    if ($data && $data->num_rows > 0) {
                        foreach ($data as $rec) {
                            ...
                            <tr>
                                ...
                                <td><?= $rec['pelabuhan'] ?></td>
                                <td><?= $rec['trip'] ?></td>
                                <td style="text-align: left">Rp <?= number_format($rec['total'], 0, ',', '.') ?></td>
                                <form action="transaksi.php?id=<?= $rec['id'] ?>" method="post">
                                ...
                                <form action="pdf.php?id=<?= $rec['id'] ?>" method="post">
                                    ...
                                    <form action="lap.php" method="post" onsubmit="return confirm('Anda Yakin Ingin Menghapus Data?');">
                                    <input type="hidden" name="id" value="<?= $rec['id'] ?>" />
                                    <td><input type="submit" value="Delete" name="delLap" class="btn btn-primary"></td>
                                </form>
                            </tr>
                            <?php   
                            $no++;
                        ...
                        echo "<tr><td colspan='7' class='text-center'>Tidak ada data untuk tanggal yang dipilih.</td></tr>";
                    }
                    ?>
